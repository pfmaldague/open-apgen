...
int ap_integral(int argc, char* argv[], const char* Pname) {
	int		i;
	XmlRpcValue	oneArg;
	...
	client().execute("getIntegratedConsumption", Args, result);
	if(!client().isFault()) {
		if(reportTime) {
			struct timeb T;
			ftime(&T);
		...
		std::cout << "\ngetIntegratedConsumption result: " << result << std::endl; }
	else {
		std::cout << "\nError calling 'getIntegratedConsumption': " << result << "\n"; }
