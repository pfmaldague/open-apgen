...
int usage(const char* Pname, const char *A) {
	std::cerr << "Usage: " << Pname << A << " [serverHost serverPort] [-v] [-t] [-log]\n";
	return -1; }

int ap_getapf(int argc, char* argv[], const char* Pname) {
	int		i;
	XmlRpcValue	oneArg;
	...
	oneArg["globals"] = "comment";
	oneArg["epochs"] = "comment";
	oneArg["time_systems"] = "comment";
	oneArg["legends"] = "comment";
	if(Verbose) {
		std::cout << "sending this message:\ngetAPF" << oneArg << std::endl; }
	if(reportTime) {
		struct timeb T;
		ftime(&T);
		startSeconds = (double) T.time;
		startSeconds += 0.001 * (double) T.millitm; }
	client().execute("getAPF", oneArg, result);
	if(!client().isFault()) {
		if(reportTime) {
			struct timeb T;
			ftime(&T);
