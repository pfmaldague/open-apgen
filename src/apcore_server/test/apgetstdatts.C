...
int ap_getstdatts(int argc, char* argv[], const char*Pname) {
	int		i;
	...
	client().execute("getStandardActivityAttributes", noArgs, result);
	if(!client().isFault()) {
		if(reportTime) {
			struct timeb T;
			ftime(&T);
		...
		std::cout << "\ngetStandardActivityAttributes result:\n" << result << std::endl;}
	else {
		std::cout << "\n Error in StandardActivityAttributes: " << result << "\n"; }
