...
XmlRpcUtil::findTag(const char* tag, std::string const& xml, int* offset)
{
  ...
  if (istart == std::string::npos) {
    // PFM added this: maybe an attribute is present; find terminating '>' with attribute present
    int l = strlen(tag);
    if(l && tag[l - 1] == '>') {
	char *dup = strdup(tag);
	dup[l - 1] = '\0';
	istart = xml.find(dup, *offset);
	if(istart == std::string::npos) {
		free(dup);
		return false; }
	*offset = int(istart + strlen(dup));
	istart = xml.find(">", *offset);
	free(dup);
	if(istart == std::string::npos) {
		return false; }
	*offset = int(istart + strlen(">"));
	return true; }
    return false; }
...
XmlRpcUtil::xmlDecode(const std::string& encoded)
{
  ...
  const char* ens = encoded.c_str();
	// debug
	// std::cerr << "xmlDecode: handling string " << encoded << std::endl;
  while (iAmp != iSize) {
    if (encoded[iAmp] == AMP && iAmp+1 < iSize) {
      int iEntity;
      int figured_it_out = 0;
	...
	if (strncmp(ens+iAmp+1, xmlEntity[iEntity], xmlEntLen[iEntity]) == 0) {
	  figured_it_out = 1;
          ...
          break; } }
      if((!figured_it_out) && *(ens + iAmp + 1) == '#') {
		const char *c = ens + iAmp + 2;
		int l = 0;
		// debug
		// std::cerr << "xmlDecode: handling char " << *c << std::endl;
		while(*c && isdigit(*c)) {
			l++;
			c++; }
		if(l && *c == ';') {
			// c is now at ens + iAmp + 2 + l, so we want to pick up at ens + iAmp + 3 + l
			char *d = (char *) malloc(l + 1);
			figured_it_out = 1;
			strncpy(d, ens + iAmp + 2, l);
			d[l] = '\0';
			decoded += (char) atoi(d);
			free(d);
			iAmp += l + 3; } }
      if (!figured_it_out) {    // unrecognized sequence
...
XmlRpcUtil::xmlEncode(const std::string& raw)
{
  ...
  std::string::size_type iSize = raw.size();
    ...
    while (iRep != iSize) {
	int iEntity;
	unsigned char C = raw[iRep];
#ifdef USELESS_IF_NO_FUNNY_CHARS_FOUND
	// PFM 8/27/2010
	if(C >= 128) {
		// debug
		std::cout << "Found illegal char " << C << "\n";
		C -= 128;
		if(!isprint(C)) {
			C = '?'; } }
#endif /* USELESS_IF_NO_FUNNY_CHARS_FOUND */
	for (iEntity = 0; rawEntity[iEntity] != 0; ++iEntity) {
		if (C == rawEntity[iEntity]) {
			// it's a special char
			encoded += AMP;
