...
void XmlRpcDispatch::setExitFlag() {
	ExitFlag = true; }
...
void XmlRpcDispatch::set_timeout_value_to(double t) {
	theTimeoutValue = t;
	// Compute end time
	// debug
	// std::cerr << "XmlRpcDispatch::set_timeout_value_to(" << t << ")...\n";
	_endTime = (theTimeoutValue < 0.0) ? -1.0 : (getTime() + theTimeoutValue); }


// Watch current set of sources and process events
void
XmlRpcDispatch::work(double t)
{
	// PFM 1/24/2007
	theTimeoutValue = t;
  // Compute end time
  _endTime = (theTimeoutValue < 0.0) ? -1.0 : (getTime() + theTimeoutValue);
	// debug
	// std::cerr << "dispatch::work(), before select(); getTime() = " << getTime()
	// 	<< "; _endTime = " << _endTime << std::endl;
  ...
  while (_sources.size() > 0) {
    ...
    int nEvents;
#ifdef HAVE_MACOS
	// a bug in select() causes the timeout option to malfunction
      nEvents = select(maxFd+1, &inFd, &outFd, &excFd, NULL);
#else
    if (theTimeoutValue < 0.0)
      nEvents = select(maxFd+1, &inFd, &outFd, &excFd, NULL);
    else 
    {
      struct timeval tv;
      tv.tv_sec = (int)floor(theTimeoutValue);
      tv.tv_usec = ((int)floor(1000000.0 * (theTimeoutValue-floor(theTimeoutValue)))) % 1000000;
	...
	// PFM fixed bug: allow for rounding error by subtracting a half-second.
	// by the way, the call to select waits a random amount of time +- a few
	// milliseconds.
    if (0 <= _endTime && getTime() > (_endTime - 0.5)) {
	// debug
	// std::cerr << "dispatch: out of select(); getTime() = " << getTime()
	// 	<< ", getTime() - _endTime = " << (getTime() - _endTime)
	// 	<< "; _endTime = " << _endTime << std::endl;
      break; }
    else if(ExitFlag) {
	break; }
	...
	// debug
	// std::cerr << "dispatch: out of loop; all done.\n";
