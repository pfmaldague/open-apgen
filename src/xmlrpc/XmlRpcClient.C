...
XmlRpcClient::writeRequest() {
	...
	int totalLength = _request.length();
	int delta = totalLength / 100;
	int bytesSentSoFar = 0;

	// Try to write the request
	// if ( ! XmlRpcSocket::nbWrite(this->getfd(), _request, &_bytesWritten))
	while(!wouldBlock) {
		if(!XmlRpcSocket::nbWriteSingle(this->getfd(), _request, &_bytesWritten, wouldBlock)) {
			XmlRpcUtil::error("Error in XmlRpcClient::writeRequest: write error (%s).",XmlRpcSocket::getErrorMsg().c_str());
			return false; }
		if(progressCallback && (_bytesWritten - bytesSentSoFar) > delta) {
			// debug
			XmlRpcUtil::log(1, "readResponse: progress = %lf",
				((double) _response.length()) / (double) _contentLength);
			bytesSentSoFar = _bytesWritten;
			progressCallback(0.5 * ((double) _bytesWritten) / (double) totalLength); }
...
XmlRpcClient::readResponse() {
	int delta = _contentLength / 100;
	if(delta == 0) delta = _contentLength;
	...
	if(int(_response.length()) < _contentLength) {
		...
		while(!wouldBlock) {
			if (!XmlRpcSocket::nbReadSingle(this->getfd(), _response, &_eof, wouldBlock)) {
				XmlRpcUtil::error("Error in XmlRpcClient::readResponse: read error (%s).",XmlRpcSocket::getErrorMsg().c_str());
				return false; }
			int afterReading = _response.length();
			if(progressCallback && (afterReading - bytesReadSoFar) > delta) {
				// debug
				XmlRpcUtil::log(1, "readResponse: progress = %lf",
					((double) afterReading) / (double) _contentLength);
				progressCallback(0.5 + 0.5 * ((double) afterReading) / (double) _contentLength); }
			bytesReadSoFar = afterReading;
			if(bytesReadSoFar == _contentLength) {
				break; } }
		...
		if(int(_response.length()) < _contentLength) {
			if (_eof) {
				...
				return false; }
			// debug
			XmlRpcUtil::log(1, "readResponse: returning after progress = %lf (next call would block)",
				((double) _response.length()) / (double) _contentLength);
...
void XmlRpcClient::unregisterCallback() {
	if(progressCallback) {
		progressCallback = NULL; } }

bool XmlRpcClient::registerCallback(void (*cb)(double)) {
  if(progressCallback) {
    return false;
  }
  progressCallback = cb;
...
XmlRpcClient::parseResponse(XmlRpcValue& result)
{
      ...
      (XmlRpcUtil::nextTagIs(FAULT_TAG,_response,&offset) && (_isFault = true)))
  {
	if ( ! result.fromXml(_response, &offset)) {
		XmlRpcUtil::log(5, "parseResponse: Bad conversion from Xml:\n%s", _response.c_str());
		XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response value. Response:\n%s",
			_response.c_str());
			_response = "";
			return false; }
	else {
		XmlRpcUtil::log(5, "parseResponse: Good conversion from Xml; result size = %d\n", result.size());
		}
  } else {
  ...
  _response = "";
  XmlRpcUtil::log(5, "parseResponse: returning %d\n", result.valid());
  return result.valid();
}
