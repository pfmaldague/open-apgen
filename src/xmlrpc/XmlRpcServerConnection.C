...
XmlRpcServerConnection::readRequest()
{
  ...
  XmlRpcUtil::log(1, "XmlRpcServerConnection::readRequest:\n%s\n", _request.c_str());
...
XmlRpcServerConnection::executeRequest()
{
  ...
  try {
	...
	// PFM: set error code to 2
      generateFaultResponse(methodName + ": unknown method name", 2);
    else
...
XmlRpcServerConnection::generateResponse(std::string const& resultXml)
{
  ...
  XmlRpcUtil::log(1, "XmlRpcServerConnection::generateResponse:\n%s\n", _response.c_str()); 
}
...
void
XmlRpcServerConnection::generateFaultResponse(std::string const& errorMsg, long errorCode)
{
  const char RESPONSE_1[] = 
    "<?xml version=\"1.0\"?>\r\n"
  ...
  faultStruct[FAULTCODE] = (long) errorCode;
  faultStruct[FAULTSTRING] = errorMsg;
  std::string body = RESPONSE_1 + faultStruct.toXml() + RESPONSE_2;
  std::string header = generateHeader(body);
  _response = header + body;
  XmlRpcUtil::log(1, "XmlRpcServerConnection::generateFaultResponse:\n%s\n", _response.c_str()); 
}
