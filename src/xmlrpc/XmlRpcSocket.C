...
bool
XmlRpcSocket::nbReadSingle(int fd, std::string& s, bool *eof, bool &wouldBlock) {
	...
	*eof = false;
	...
	// while ( ! wouldBlock && ! *eof) {
		...
		if (n > 0) {
			readBuf[n] = 0;
			s.append(readBuf, n); } else if (n == 0) {
			*eof = true; }
...
XmlRpcSocket::nbWrite(int fd, std::string& s, int *bytesSoFar)
{
  ...
  while ( nToWrite > 0 && ! wouldBlock ) {
	...
	// PFM 11/29/2005
	;
	// there is no sign of the code ever trying again, so let's try now
      // wouldBlock = true;
    } else {
      return false;   // Error
    }
...
// same as above but returns every time a chunk is sent
bool 
XmlRpcSocket::nbWriteSingle(int fd, std::string& s, int *bytesSoFar, bool &wouldBlock) {
	int nToWrite = int(s.length()) - *bytesSoFar;
	char *sp = const_cast<char*>(s.c_str()) + *bytesSoFar;
	// bool wouldBlock = false;
	...
	// while ( nToWrite > 0 && ! wouldBlock ) {
		...
		int n = write(fd, sp, nToWrite);
#endif
		XmlRpcUtil::log(5, "XmlRpcSocket::nbWriteSingle: send/write returned %d.", n);
